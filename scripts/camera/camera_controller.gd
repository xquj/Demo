...
# 邪恶冥刻风格：围绕桌面可做 WASD 四向观察。
const PRESET_BOARD_FRONT := {
	"length": 3.0,
	"rot_x": 6.0,
	"rot_y": -7.0,
	"rot_z": 2.2,
...
func _get_preset(state: CameraState) -> Dictionary:
	match state:
		CameraState.BOARD_FRONT:
			return PRESET_BOARD_FRONT
		CameraState.BOARD_LEFT:
			return PRESET_BOARD_LEFT
		CameraState.BOARD_RIGHT:
			return PRESET_BOARD_RIGHT
		CameraState.BOARD_BACK:
			return PRESET_BOARD_BACK
		_:
			return PRESET_HAND
