...
func enter() -> void:
	...
	if card.team_id == global.local_player.team_id:
		...
		target_pos = Vector3(global.multi_player_node.global_position.x - 0.125 + idx * 0.2,global.multi_player_node.global_position.y + 0.01,global.multi_player_node.global_position.z)
		target_rot = Vector3(card.rotation.x + deg_to_rad(45),card.rotation.y  + deg_to_rad(180),card.rotation.z)
		card.reparent(global.multi_player_node)
