extends Object
class_name VectorAnimationUtils

# 你指定的核心成员变量（严格保留，补充类型注解和注释）
var time: int = 0          # 已运行时间（毫秒，与end_time单位一致）
var end_time: int = 500    # 动画总时长（毫秒，默认500ms，可自定义）
var value: Vector2         # 动画当前计算结果值（实时更新，外部直接读取）
var start_value: Vector2   # 动画起始值
var end_value: Vector2     # 动画目标值
...
var _temp_start: Vector2 = Vector2(0.0,0.0)  # 临时保存起始值（乒乓循环/重置用）

# 构造方法（可选，快速初始化动画参数）
func _init(_start: Vector2 = Vector2(0.0,0.0), _end: Vector2 = Vector2(1.0,1.0), _duration: int = 500, _ease: EaseType = EaseType.LINEAR) -> void:
	start_value = _start
	end_value = _end
	end_time = _duration
	ease_type = _ease
...
func calc_by_progress(progress: float) -> Vector2:
...
func set_target(new_end_value: Vector2, new_duration: int = -1) -> void:
